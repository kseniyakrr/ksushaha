...
namespace WindowsFormsApp2
{
    public partial class Form1 : Form
    {
        ...
        private void richTextBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            isTextChanged = richTextBox1.Text != lastSavedContent;
        ...
        {
            // Проверяем, есть ли несохраненные изменения
            if (isTextChanged && richTextBox1.Text != lastSavedContent)
            {
                ...
                switch (dialogResult)
                {
                    case DialogResult.Yes:
                        ...
                        {
                            СохранитьКак_Click(sender, e);
                            // Если после "Сохранить как..." флаг изменений остался true
                            if (isTextChanged)
                                return;
                        }
                        break;
            ...
            using (OpenFileDialog openFileDialog1 = new OpenFileDialog())
            {
                ...
                if (openFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        lastSavedContent = fileContent; // Запоминаем содержимое нового файла
        ...
        private void Отменить_Click(object sender, EventArgs e)
        {
            if (undoStack.Count > 0)
            {
                ...
                else if (lastOperation.Type == "Insert")
                {
                    // Отмена вставки: удаляем по одному символу с конца вставленного текста
                    if (lastOperation.Text.Length > 0)
                    {
                        // Получаем позицию конца вставленного текста
                        int endPosition = lastOperation.Position + lastOperation.Text.Length;

                        // Удаляем последний символ
                        string removedChar = lastOperation.Text.Substring(lastOperation.Text.Length - 1, 1);

                        ignoreTextChanged = true;
                        richTextBox1.Text = richTextBox1.Text.Remove(endPosition - 1, 1);
                        ignoreTextChanged = false;

                        // Создаем операцию для redo
                        EditOperation redoOperation = new EditOperation
                        {
                            Type = "Insert",
                            Text = removedChar,
                        ...
                        // Если остались символы для отмены, возвращаем операцию в стек
                        if (lastOperation.Text.Length > 1)
                        {
                            EditOperation remainingOperation = new EditOperation
                            {
                                Type = "Insert",
                                Text = lastOperation.Text.Substring(0, lastOperation.Text.Length - 1),
                                Position = lastOperation.Position
                            };
                            undoStack.Push(remainingOperation);
                        }
                    }
                }
                else if (lastOperation.Type == "Delete")
                {
                    // Отмена удаления: вставляем удаленный текст обратно
                    ignoreTextChanged = true;
                ...
                {
                    // Стандартная отмена для одиночных операций
                    string currentText = richTextBox1.Text;
        ...
        {
            string htmlFilePath = @"C:\Users\kskor\Downloads\данилу (3)\данилу\About.html";
        ...
        {
            string htmlFilePath = @"C:\Users\kskor\Downloads\данилу (3)\данилу\Help.html";
        ...
        private void тестовыйПримерToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (File.Exists(filePath))
                {
                    ...
                    if (richTextBox1.Text != testCaseText)
                    {
        ...
        {
            string htmlFilePath = @"C:\Users\kskor\Downloads\данилу (3)\данилу\Grammar - Копировать.html";
            ...
            try
            {
                ...
                // Создаем форму для отображения кода
                Form codeForm = new Form();
                codeForm.Text = "Исходный код программы";

                // Используем RichTextBox с моноширинным шрифтом
                RichTextBox rtb = new RichTextBox();
                rtb.Dock = DockStyle.Fill;
                rtb.Font = new Font("Consolas", 10);
                rtb.ReadOnly = true;

                // Если нужно извлечь код из HTML (простой вариант)
                string plainText = System.Text.RegularExpressions.Regex.Replace(
                    htmlContent, "<[^>]*>", string.Empty);
                rtb.Text = plainText;

                codeForm.Controls.Add(rtb);
                codeForm.Show();
            }
            catch (Exception ex)
            {
